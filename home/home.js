...
/* ───────────────────────────────────────────────────────────────────────────
   macOS Tahoe enhancements — auto-hiding dock, active section, cursor trail
   ─────────────────────────────────────────────────────────────────────────── */
(() => {
    // ─── Dock: auto-hide on scroll down, show on scroll up / hover top edge ───
    ...
    if (dock) {
        ...
        // Show dock when cursor near top edge
        window.addEventListener('mousemove', (e) => {
            if (e.clientY < 60) dock.classList.remove('is-hidden');
        }, { passive: true });

        // ─── Active section highlighting in dock ───
        const sectionIds = ['hero', 'content', 'skin', 'videos', 'about'];
        const sections = sectionIds
            .map(id => ({ id, el: document.getElementById(id) }))
            .filter(s => s.el);
        const dockItems = dock.querySelectorAll('.dock__item[data-target]');
    ...
    // ─── Glowing cursor trail (desktop hover-capable only) ───
    const canHover = window.matchMedia('(hover: hover) and (pointer: fine)').matches;
    const trail = document.querySelector('.cursor-trail');
    if (canHover && trail) {
        const dot = trail.querySelector('.cursor-trail__dot');
        const halo = trail.querySelector('.cursor-trail__halo');
        let mx = -100, my = -100;
        let hx = -100, hy = -100;
        ...
        const hoverSel = 'a, button, .btn, .icon-btn, .mini-button, .dock__item, .carousel .card, summary, .cell';
    ...
    // ─── Smooth-scroll for dock items targeting same-page anchors ───
    document.querySelectorAll('.dock__item[href^="#"]').forEach(a => {
